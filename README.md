...
- `basket:add` — добавить товар в корзину (`{ id }`)
- `basket:remove` — удалить товар из корзины (`{ id }`)

#### Товары:

- `product:show-details` — открыть детали товара (`{ id }`)

#### Оформление заказа:

- `order:payment` — открыть экран оплаты
- `order:contacts` — открыть экран ввода контактов
- `order:success` — открыть экран успешного заказа
- `order:success-close` — закрыть экран успешного заказа
- `order:submit` — отправить заказ

#### Формы заказа:

- `order:payment-change` — изменение способа оплаты (`{ payment }`)
- `order:address-change` — изменение адреса (`{ address }`)
- `order:email-change` — изменение email (`{ email }`)
- `order:phone-change` — изменение телефона (`{ phone }`)

#### Прочее:

- `order:contacts` — переход к форме контактов
